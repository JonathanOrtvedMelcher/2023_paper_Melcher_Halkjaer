...
color_bar_ax = None):
    ...
    if add_true_values:
        ...
        for i, E_P in enumerate(zip(E_for_plot_lohmann,P_for_plot_lohmann)):
            ...
            color = color_lohmann_10k, fontsize = 20, zorder = 100)
...
cbar_ax_1 = fig.add_axes([0.13,0.07,.35,0.01])
cbar_ax_2 = fig.add_axes([0.54,0.07,.35,0.01])
...
for i, alpha_index_ in enumerate(alpha_index_for_sidebyside):
    ...
    ax[i][0].set_ylabel(r"$\mu'_E$" + ' [events per 20 kyr]', fontsize = 20)
    if i == 0:
    # add text on the right side of the plot with the alpha value
        ...
        transform=ax[i][1].transAxes, fontsize = 20, rotation = 270)
    else:
        # add text on the right side of the plot with the alpha value
        ax[i][1].text(1.02, 0.5, r'$\alpha$ =',
        horizontalalignment='center', verticalalignment='center', 
        ...
        ax[i][1].text(1.05, 7 ,r'$-$', fontsize=24, rotation=270)

        ax[i][1].text(1.017, 0.33,  
        f'{np.abs(np.round(np.unique(alpha_)[alpha_index_], 2))}',
        horizontalalignment='center', verticalalignment='center', 
        transform=ax[i][1].transAxes, fontsize = 20, rotation = 270)
...
ax[-1][0].set_xlabel(r"$\mu'_P$" , fontsize=20)
ax[-1][1].set_xlabel(r"$\mu'_P$" , fontsize=20)
...
for ax_sub in ax.flatten():
    ...
    if ax.flatten().tolist().index(ax_sub) % 2 == 1:
        x = -0.14
    ax_sub.text(x,y, '(' + \
    string.ascii_lowercase[ax.flatten().tolist().index(ax_sub)] + ')', fontsize=25)
